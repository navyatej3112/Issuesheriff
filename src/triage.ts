import { Config, IssueContext, PRContext } from './types';
...
export function detectLabels(
  ...
  if (config.keywords?.bug) {
    const hasBugKeyword = config.keywords.bug.some(keyword =>
  ...
  if (config.keywords?.feature) {
    const hasFeatureKeyword = config.keywords.feature.some(keyword =>
      text.includes(keyword.toLowerCase())
    );
  ...
  if (config.keywords?.docs) {
    const hasDocsKeyword = config.keywords.docs.some(keyword =>
      text.includes(keyword.toLowerCase())
    );
  ...
  if (config.keywords?.priority) {
    const hasPriorityKeyword = config.keywords.priority.some(keyword =>
      text.includes(keyword.toLowerCase())
    );
...
export function detectAreaLabels(
  files: string[],
  config: Config
): string[] {
  const labels: string[] = [];
  ...
  if (config.paths?.backend && config.areaLabels?.backend) {
    const hasBackendFile = files.some(file =>
      config.paths!.backend!.some(path => file.startsWith(path))
    );
    if (hasBackendFile) {
  ...
  if (config.paths?.frontend && config.areaLabels?.frontend) {
    const hasFrontendFile = files.some(file =>
      config.paths!.frontend!.some(path => file.startsWith(path))
    );
    if (hasFrontendFile) {
  ...
  if (config.paths?.infra && config.areaLabels?.infra) {
    const hasInfraFile = files.some(file =>
      config.paths!.infra!.some(path => file.startsWith(path))
    );
    if (hasInfraFile) {
...
export function needsInfo(title: string, body: string): boolean {
  const text = `${title} ${body}`.toLowerCase();
  ...
  const hasExpectedBehavior =
    text.includes('expected') && text.includes('behavior') ||
    text.includes('expected') && text.includes('result') ||
    text.includes('should') && text.includes('but');
  
  const hasActualBehavior =
    text.includes('actual') && text.includes('behavior') ||
...
export function generateNeedsInfoComment(): string {
